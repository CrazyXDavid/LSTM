...
model = Sequential()
model.add(Dense(50))
model.add(LSTM(25,activation='relu', return_sequences=False, input_shape = (n_steps, n_features)))
model.add(Dense(1))
#model.summary()
...
model.save(f'models/epochs{epochs}_{n_steps}_{n_features}_D50_L25_D1')
...
print((final['Predict'].mean()-final['Real'].mean()) / final['Real'].mean() * 100, '%')
...
# 預計 - 實際
a = pd.concat([nd['close'][start:end], df['close'][start:end]],axis=1)
nd['close'][start-30:end+30].plot(color='#00ff00') # 預計
df['close'][start-30:end+30].plot(color='#FF0000') # 實際
print(nd['close'][start+300], df['close'][start+300])
# a.plot()